...
class PropiedadesActividad(db.Model):
    ...
    documentacion: Mapped[str | None]  = mapped_column(String(200), nullable=True)
    cartel_promocional: Mapped[str | None]  = mapped_column(String(200), nullable=True)
    cupo_maximo: Mapped[int | None]  = mapped_column(SmallInteger, nullable=True)
    referencias: Mapped[str | None]  = mapped_column(String(5000), nullable=True)
